...
// Rate limiters for sensitive auth endpoints (prevent brute-force & email spam)
...
// Only sends a reset email if the account actually exists & is active.
router.post('/forgot-password', forgotLimiter, [
  ...
  try {
    ...
    // If the email is not registered (or inactive), do NOT send anything.
    if (!user) {
    ...
    db.prepare(`INSERT INTO activity_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)`)
      .run(user.id, 'Forgot Password', 'user', user.id, `Password reset requested for ${user.email}`);

    // Respond immediately
    res.json({ message: 'A password reset link has been sent to your email' });

    // Send email in background (non-blocking) to the REAL user email
...
router.post('/reset-password', [
  ...
  try {
    ...
    db.prepare('UPDATE users SET password_hash = ?, reset_token = NULL, reset_token_expiry = NULL, is_active = 1 WHERE id = ?')
      .run(hash, user.id);

    db.prepare(`INSERT INTO activity_log (user_id, action, target_type, target_id, details) VALUES (?, ?, ?, ?, ?)`)
      .run(user.id, 'Reset Password', 'user', user.id, 'User reset their password via email link');
...
router.get('/me', require('../middleware/auth'), (req, res) => {
  try {
    const user = db.prepare('SELECT id, name, email, role, is_active, created_at, email_notifications FROM users WHERE id = ?').get(req.user.id);
...
router.put('/notification-preferences', require('../middleware/auth'), (req, res) => {
  const { email_notifications } = req.body;
  try {
    const val = email_notifications ? 1 : 0;
    db.prepare('UPDATE users SET email_notifications = ? WHERE id = ?').run(val, req.user.id);
