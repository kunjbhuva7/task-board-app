...
// GET /api/office-expenses — list with optional filters
...
// GET /api/office-expenses/summary — totals
router.get('/summary', async (req, res) => {
  try {
    const { from, to } = req.query;
    let sql = 'SELECT status, COUNT(*) as count, COALESCE(SUM(amount),0) as total FROM office_expenses WHERE user_id = $1';
    const params = [req.user.id];
    let idx = 2;
    if (from) { sql += ` AND expense_date >= $${idx++}`; params.push(from); }
    if (to) { sql += ` AND expense_date <= $${idx++}`; params.push(to); }
    ...
    const allRow = await db.get('SELECT COALESCE(SUM(amount),0) as total, COUNT(*) as count FROM office_expenses WHERE user_id = $1' + (from ? ` AND expense_date >= '${from}'` : '') + (to ? ` AND expense_date <= '${to}'` : ''), [req.user.id]);

    const summary = { total: Number(allRow.total), count: Number(allRow.count), pending: 0, approved: 0, rejected: 0 };
    for (const r of rows) {
      if (r.status === 'pending') summary.pending = Number(r.total);
      if (r.status === 'approved') summary.approved = Number(r.total);
      if (r.status === 'rejected') summary.rejected = Number(r.total);
    }
    res.json(summary);
  } catch (e) { console.error(e); res.status(500).json({ message: 'Server error' }); }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const { expense_date, item, amount, category, payment_mode, notes, status } = req.body;
    await db.run(
      'UPDATE office_expenses SET expense_date=$1, item=$2, amount=$3, category=$4, payment_mode=$5, notes=$6, status=$7 WHERE id=$8',
      [expense_date || existing.expense_date, item || existing.item, amount != null ? Number(amount) : existing.amount, category || existing.category, payment_mode || existing.payment_mode, notes !== undefined ? notes : existing.notes, status || existing.status, existing.id]
    );
